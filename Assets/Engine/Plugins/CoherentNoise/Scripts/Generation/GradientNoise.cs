...
namespace Engine.Plugins.CoherentNoise.Scripts.Generation
{
	...
	/// </summary>
	public class GradientNoise : Generator
	{
		// 256 random vectors
		private static readonly float[] s_vectors =
		{
		...
		Vector3 GetRandomVector(int x, int y, int z)
		{
			if (Period > 0)
			{
			...
			int vectorIndex = (
			   ...
			   & 0x7fffffff;
			vectorIndex = (((vectorIndex >> Constants.ValueShift) ^ vectorIndex) & 0xff) * 3;

			return new Vector3(s_vectors[vectorIndex], s_vectors[vectorIndex + 1], s_vectors[vectorIndex + 2]);
		}

		private float GetNoise(float x, float y, float z, int ix, int iy, int iz)
		{
			var gradient = GetRandomVector(ix, iy, iz);
			return Vector3.Dot(gradient, new Vector3(x - ix, y - iy, z - iz)) * 2.12f; // scale to [-1,1]
		}
