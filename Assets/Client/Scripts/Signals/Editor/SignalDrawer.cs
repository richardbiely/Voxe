...
using Client.Scripts.Signals;
using UnityEditor;
using UnityEngine;

namespace Assets.Client.Scripts.Signals.Editor
{
    ...
    public class SignalDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var prefixRect = new Rect(position.x + indent, position.y + position.height/3,
                (position.width - indent)*0.25f, position.height/3);
            var valueRect = new Rect(prefixRect.x + prefixRect.width, prefixRect.y, (position.width - indent)*0.75f,
                position.height/3);

            var targetProperty = property.FindPropertyRelative("target");
            GUI.Label(prefixRect, "Target", EditorStyles.miniLabel);
            targetProperty.objectReferenceValue = EditorGUI.ObjectField(valueRect, targetProperty.objectReferenceValue,
            ...
            if (obj != null)
            {
                ...
                for (int i = 0; i < components.Length; i++)
                {
                    ...
                    for (int j = 0; j < methods.Length; j++)
                    {
                        ...
                        var attr = (SignalAttribute) attributes[0];
